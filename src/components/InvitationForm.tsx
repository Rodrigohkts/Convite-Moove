...
export default function InvitationForm() {
    ...
    // Check registration count on mount
    useEffect(() => {
    ...
    if (countLoading) {
        return (
            <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-8 sm:p-10 shadow-2xl relative flex flex-col items-center justify-center h-[460px]">
                <Loader2 className="w-8 h-8 text-moove-green animate-spin mb-4" />
                <p className="text-gray-400 text-sm">Verificando disponibilidade...</p>
            </div>
        );
    }
    ...
    if (isLimitReached) {
        return (
            <div className="bg-[#0f0f12] border border-white/10 rounded-2xl p-8 sm:p-10 shadow-2xl relative flex flex-col items-center justify-center text-center h-[460px] animate-in fade-in zoom-in duration-500 overflow-hidden">
                ...
                <h3 className="font-sans font-bold tracking-tight text-3xl text-white mb-2 ml-1">Vagas Esgotadas!</h3>
                <p className="text-gray-400 font-sans font-medium mb-6">
                    Infelizmente o limite de participantes para este encontro foi atingido. Fique atento às nossas próximas edições.
                </p>
                
                <button 
                    disabled
                    className="w-full bg-gray-800 text-gray-500 font-semibold tracking-wide text-[15px] rounded-xl py-3.5 mt-4 cursor-not-allowed border border-white/5"
                >
